...
type SweepReference = Mutex<BackgroundSweep>;

pub struct GC {
    background_sweep: Mutex<BackgroundSweep>,
    memory_heap: MemoryHeap,
}

pub fn new_gc() -> GC {
    GC {
        background_sweep: Mutex::new(new_sweep()),
...
impl GC {
    ...
    async fn background_sweep(&mut self, signal_setup_done: oneshot::Sender<bool>) {
        ...
        let mut gc_ready = {
            let mut sweep = self.background_sweep.lock().expect("Could not get lock on SweepData");
            sweep.park();
    ...
    pub fn sweep_one(&mut self) -> usize {
        ...
        if self.memory_heap.sweep_done.load(Ordering::Relaxed) != 0 {
    	 	!0
    	} else {
            0
        }
