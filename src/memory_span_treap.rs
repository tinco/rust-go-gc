...
//
//go:notinheap
pub struct MemorySpanTreap {
	pub treap: *mut MemorySpanTreapNode
}

pub struct MemorySpanTreapNode {
	right:     *mut MemorySpanTreapNode, // all treapNodes > this treap node
	left:      *mut MemorySpanTreapNode, // all treapNodes < this treap node
	parent:    *mut MemorySpanTreapNode, // direct parent of this node, nil if root
	number_of_pages_in_span_key: usize,    // number of pages in spanKey, used as primary sort key
	span_key:   MemorySpan,     // span of size npagesKey, used as secondary sort key
	priority:  u32,     // random number used by treap algorithm to keep tree probabilistically balanced
}

impl MemorySpanTreap {
    pub fn new() -> MemorySpanTreap {
        MemorySpanTreap {
            treap: std::ptr::null_mut(),
        }
    }

    pub fn remove_span(&mut self, span: &mut MemorySpanData) {
        unimplemented!()
    }
}
